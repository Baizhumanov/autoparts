<?
	...
	$antifreezes = getAntifreeze($connection);
?>
...
<body>
	...
	<main>
		<div class="container">
			<div class="row my-5">
				<div class="col title text-center">Aнтифриз каталогі</div>
			</div>
			<div class="row justify-content-center grid my-5">
				<? for ($i = 0; $i < count($antifreezes); $i++): ?>
				<div class="col-10 col-sm-6 col-md-4 col-lg-3">
					<div class="item" id="<? echo $antifreezes[$i]["id"] ?>">
						<div class="name"><? echo $antifreezes[$i]["name"]; ?></div>
						<div class="content">
							<div class="additional">
								<div class="info">Сыйымдылық: <? echo $antifreezes[$i]["tank"]; ?></div>
								<div class="info">Спец.: <? echo $antifreezes[$i]["spec"]; ?></div>
							</div>
							<div class="price" id="price-<? echo $antifreezes[$i]["id"] ?>">Бағасы: <? echo $antifreezes[$i]["price"]; ?></div>
							<div class="count">
								<button onclick="onClick(<? echo $antifreezes[$i]["id"] ?>, '-')"><img src="../icons/minus.svg"></button>
								<div class="text" id="count-<? echo $antifreezes[$i]["id"] ?>">0</div>
								<button onclick="onClick(<? echo $antifreezes[$i]["id"] ?>, '+')"><img src="../icons/plus.svg"></button>
							</div>
							<div class="sum" id="sum-<? echo $antifreezes[$i]["id"] ?>">0</div>
						</div>
					</div>
				</div>
				<? endfor; ?>
		...
		function onClick(id, o) {
			...
			if (o == "-") {
				if (text > 0) {
					...
					mainCount--;
					if (text == 0) {
						localStorage.removeItem("ai-" + id);
						item.className = "item";
					} else {
						localStorage.setItem("ai-" + id, text + "+" + name + "+" + price);
					}
				}
			} else {
				if (text != 10) {
		...
		function onLoad() {
			...
			for (var i = 1; i < items.length; i+=2) {
				for (var j = 0; j < localStorage.length; j++) {
					...
					if ("ai-" + items[i].childNodes[1].id == key) {
